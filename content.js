...
// Initialize on load
chrome.storage.local.get(['isRunning', 'scrollPixel', 'scrollTime', 'autoRefresh', 'refreshInterval'], (result) => {
    if (result.isRunning) {
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'start') {
        chrome.storage.local.get(['scrollPixel', 'scrollTime', 'autoRefresh', 'refreshInterval'], (result) => {
            startScrolling(result);
            if (result.autoRefresh) {
                scheduleRefresh(result.refreshInterval);
            }
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
        ...
        chrome.storage.local.get(['isRunning', 'scrollPixel', 'scrollTime', 'autoRefresh', 'refreshInterval'], (result) => {
            if (!result.isRunning) {
                console.log('[Infinite Scroll] Resumed by ENTER key');
...
function scheduleRefresh(intervalSeconds) {
    ...
    console.log(`[Infinite Scroll] Auto refresh scheduled in ${intervalSeconds}s`);
